...
class PdfToTiffConverter internal constructor(
    ...
    suspend fun convert(inputFile: File, outputFile: File) {
        ...
        val pdfReadFlow = apexFlow<Pair<File, File>, Pair<File, Flow<BufferedImage>>> {
            transformOnIO { (pdfFile, tiffFile) ->
                val imagesFlow = PdfImageReader(pdfFile, pdfConfig).read()
        ...
        val tiffWriteFlow = apexFlow<Pair<File, Flow<BufferedImage>>, Unit> {
            transformOnIO { (tiffFile, imagesFlow) ->
                TiffWriter(tiffFile, tiffConfig).write(imagesFlow)
            }
        }
    ...
    suspend fun convert(inputStream: InputStream, outputStream: OutputStream) {
        // Create PDF read flow (IO intensive)
        val pdfReadFlow =
            apexFlow<Pair<InputStream, OutputStream>, Pair<OutputStream, Flow<BufferedImage>>> {
                transformOnIO { (input, output) ->
                    val imagesFlow = PdfImageReader(input, pdfConfig).read()
                        .withTiming("dev.waylon.apexflow.pdf.reader")
                        .flowOn(Dispatchers.IO)
        ...
        // Create TIFF write flow (IO intensive)
        val tiffWriteFlow = apexFlow<Pair<OutputStream, Flow<BufferedImage>>, Unit> {
            transformOnIO { (output, imagesFlow) ->
                TiffWriter(output, tiffConfig).write(imagesFlow)
            }
        }
            .withTiming("TIFF Writing Phase")
            .withLogging("TIFF Writing")
        ...
        pdfToTiffFlow.execute(inputStream to outputStream).collect { }
    }
